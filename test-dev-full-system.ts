...
async function runTest() {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    console.log(`  [${(i+1)*10}s] CPU: ${mon.cpu}% | MEM: ${mon.memory}% | Cycles: ${currentCycles} | Rate: ${cyclesPerMinute}/min | Indications: ${(Object.values(connLog.summary?.indicationsCounts || {}) as any[]).reduce((a: number, b: number) => a + b, 0)}`);
  ...
  RESULTS.summary = {
    ...
    totalIndications: (Object.values(finalConn.summary?.indicationsCounts || {}) as any[]).reduce((a: number, b: number) => a + b, 0),
